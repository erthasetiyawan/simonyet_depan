...
class BaseController
{
    ...
    {
        // warna : lazur-bg, yellow-bg, red-bg, navy-bg
        ...
        $data = [
            ...
            'widget4' => [
                ...
                'label' => 'Total Sewa',
                'data' => number_format($param['total_sewa']['total_sewa'],0,',','.')
            ],
	...
	{
		$param = [
			'jumlah_aset' => count($this->pushArray("select * from aset")),
			'jumlah_sewa' => count($this->pushArray("select * from sewa")),
			'jumlah_pengguna' => count($this->pushArray("select * from pengguna where level = 2")),
			'total_sewa' => $this->toArray("select sum(total_harga_sewa) as total_sewa from sewa")
		];

		$data = [
			'widget' => $this->widget($param)
		];

		View::render('App.view.home.index', $data);
	}
